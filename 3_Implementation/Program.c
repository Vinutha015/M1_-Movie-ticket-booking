...
void insert_details()
{
	...
	{
		fprintf(fp,"%s %s %s %d %s \n",b.code,b.name,b.date,b.cost,b.language);
...
void find() 
{
	...
	{	
		while(getc(fp) != EOF)
		{
			fscanf(fp,"%s %s %s  %d %s",b.code,b.name,b.date,&b.cost,b.language);
...
void book_ticket()
{
 struct book b;
	...
	printf("\n For Book ticket Choose Movie(Enter Movie Code First Latter Of Movie)\n");
	printf("\n Enter movie code :");
	scanf("%s",movie_code);
	//system("clear");
	fp = fopen("data.txt","r");
	...
	{
		fprintf(ufp,"%s %d %d %d %s %d %s \n",name,mobile,total_seat,total_amount,b.name,b.cost,b.language);
